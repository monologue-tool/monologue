...
## Updates property connection tracking when a connection is made
func register_connection(
	...
	# Track the connection in both properties
	from_prop.add_connection_to(to_node_id, to_prop.name, to_port)
	to_prop.add_connection_from(from_node_id, from_prop.name, from_port)


## Updates property connection tracking when a connection is removed
func unregister_connection(
	...
	# Remove the connection from both properties
	from_prop.remove_connection_to(to_node_id, to_port)
	to_prop.remove_connection_from(from_node_id, from_port)
...
func get_connected_node(node: InspectableNode, property_name: String) -> InspectableNode:
	...
	if not prop or not prop.is_connected():
		return null
		...
		for prop: Property in node.get_properties():
			...
			for conn in prop.connected_to:
				...
				if not target_node or not target_node.get_property(conn["property_name"]):
					push_warning("Invalid connection found from %s.%s" % [node.get_property_value("id"), prop.name])
					all_valid = false
			...
			for conn in prop.connected_from:
				...
				if not source_node or not source_node.get_property(conn["property_name"]):
					push_warning("Invalid connection found to %s.%s" % [node.get_property_value("id"), prop.name])
					all_valid = false
